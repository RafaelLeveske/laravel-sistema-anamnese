                ...
                <div>
                    <li class="list-group-item   justify-content-left">
                        ...
                        <div class="listdiv d-flex mb-2">
                            <a href="/consulta/{{ $paciente->id }}/create" title="Criar Nova Consulta" class="btn btn-info btn-sm mr-1"><i class="fas fa-notes-medical"></i></a>

                            <a href="/consulta/profile/{{ $paciente->id }}/index" title="Consultas Cadastradas" class="btn btn-info btn-sm mr-1"><i class="fas fa-file-medical"></i></a>

                            <a href="/paciente/{{ $paciente->id }}/update" title="Atualizar Dados do Paciente" class="btn btn-info btn-sm mr-1"><i class="fas fa-edit"></i></a>

                            <form method="post" action="/paciente/{{ $paciente->id }}" onsubmit="return confirm('O paciente será excluido deseja prosseguir?')">
                                @csrf
                                @method('DELETE')
                                <button class="btn btn-danger" title="Excluir Paciente"><i class="far fa-trash-alt"></i></button>
                            </form>
                        </div>
                    </li>
            @endforeach
